...
const forgotPassword = async (req, res) => {
  try {
    ...
    if (!user) res.status(404).json({ status: 404, error: true, message: "El usuario no se encuentra registrado" });
      ...
      // Generar un Initialization Vector (IV) aleatorio
      const iv = Buffer.alloc(16, 0);

      // Generar la clave utilizando scryptSync
      const key = scryptSync(process.env.SECRET, 'salt', 24);

      // Crear el cipher utilizando la clave y el IV
      const cipher = createCipheriv('aes-192-cbc', key, iv);

      let encryptedToken = cipher.update(token, 'utf8', 'hex');
      encryptedToken += cipher.final('hex');

      sendEmail(user.email, 'Reset Password', message.MessageResetPassword(user.name, encryptedToken));
